...
import { Result, textToScript, } from '@zsnout/ithkuil/script';
import {
...
export async function render(text: string, font: Font, spacing: boolean = config.rendering.spaceBetweenWords): Promise<AttachmentBuilder> {
    ...
    const dataURL = await drawCharsFromObjects(parserObjects, font);
    const base64 = dataURL.replace(/^data:image\/png;base64,/, '');
	const pngBuffer: Buffer = Buffer.from(base64, "base64");
...
async function drawCharsFromObjects(parserObjects: Array<Result<any>>, font: Font): Promise<string> {
...
export async function textToPng(fontCompatibleString: string, font: Font = 'basic'): Promise<string> {
    ...
    // borderColor: '#3780C8',
	// 	padding: 10,
	const pngBytes = await generate(fixed, {
		textColor: 'white',
        bgColor: '#00000000',
        fontSize: 100,
        margin: 3,
		fontPath: config.fonts[font].path,
		fontFamily: config.fonts[font].fontName,
	});
	return pngBytes;
}
